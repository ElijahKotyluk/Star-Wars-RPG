...
function renderOpponents (selectedCharacterKey) {
  ...
  for (var i = 0; i < characterKeys.length; i++) {
    if (characterKeys[i] !== selectedCharacterKey) {
      ...
      $('#pick-enemy-container').append(enemyDiv);
    }
  }
}
...
function isGameWon () {
  console.log('Checking to see if you won or lost...');
  return gameState.enemiesLeft === 0;
}

function isAttackPhaseComplete () {
  if (isCharacterDead(gameState.selectedCharacter)) {
    alert('You were defeated by ' + gameState.selectedDefender.name + '. Click the reset button to play again.');
    ...
    if (isGameWon()) {
      alert('You win. Click the reset button to play again...');
      $('#reset-button').show();
    }
    else {
...
function emptyDivs () {
  $('#selected-character').empty();
  $('#defender').empty();
  $('#pick-enemy-container .enemy').empty();
  $('#pick-character').empty();
